...
- Core2 speaker tests use `PCMFlowDevice`'s `M5SpeakerBufferedPlayer` for M5Unified speaker queue and buffer-lifetime management.
- PCMFlowUDP owns packet-jitter absorption; PCMFlowDevice owns `playRaw()` async output buffering. See "Receive Buffering And Latency Responsibility" in `SPEC.md`.
