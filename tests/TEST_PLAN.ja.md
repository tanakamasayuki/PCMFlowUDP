...
受信バッファと遅延の全体方針は [SPEC.ja.md](../SPEC.ja.md) の「受信バッファと遅延の責務」に従う。Core2 speaker 系 manual テストでは、20 ms packet を受信し、transport 側 PCM buffering には `RtpReceiver::hardwareSpeakerPcmBuffer()`、M5Unified speaker queue と buffer lifetime 管理には `PCMFlowDevice` の `M5SpeakerBufferedPlayer` を使う。これは低遅延 VoIP 設定ではなく、実機 speaker 確認を安定させるための設定である。
