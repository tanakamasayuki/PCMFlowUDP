...
void loop()
{
    ...
    if (!g_streaming)
    {
        ...
        if (!g_sender.begin(peerIp, peerPort, kSsrc) ||
            !g_sender.setFormat({kSampleRate, 1, 16}))
        {
            Serial.println("FAIL sender-begin");
            return;
        }
        g_sender.setSequenceNumber(0);
        g_sender.setTimestamp(0);

        Serial.print("RTP-TX pt=");
        Serial.print(g_sender.payloadType());
        Serial.print(" rate=");
    ...
    Serial.print("RTP-TX packets=");
    Serial.print(g_packets);
    Serial.print(" frames=");
    Serial.print(written);
    Serial.print(" nextSeq=");
    Serial.print(g_sender.sequenceNumber());
    Serial.print(" nextTs=");
    Serial.print(g_sender.timestamp());
    Serial.print(" s0=");
